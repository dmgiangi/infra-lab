...
google first login flow: link existing user after local password re-authentication
```
...
The Google first login flow is configured to reject automatic user creation. On the first Google login, Keycloak detects the matching local user and requires local password re-authentication before linking the Google identity. This keeps Google access limited to users that were already created with local credentials.
...
```text
keycloak-configure-v4
```

When the Job logic or managed configuration must be re-applied through GitOps after a completed run, bump the Job name, for example to `keycloak-configure-v5`.
